...
def all_cnn(inputs):
  ...
  x = Conv2D(192, kernel_size=(3,3), activation='relu', padding='same')(x)
  x = BatchNormalization(axis=-1)(x)
  x = Conv2D(192, kernel_size=(3,3), activation='relu', padding='same', strides=2)(x)
  x = BatchNormalization(axis=-1)(x)
...
if __name__ == "__main__":
  ...
  _ = compile_and_train(all_cnn_model, 200, train_dataset, train_labels, valid_dataset, valid_labels)

  test_loss, test_acc = all_cnn_model.evaluate(test_dataset, test_labels)
