...
from keras.layers import Conv2D, GlobalAveragePooling2D, Activation, MaxPooling2D, Dropout, BatchNormalization
...
def nin_cnn(inputs):
  # mlpconv block 1
  x = Conv2D(32, kernel_size=(5,5), activation='relu', padding='valid')(inputs)
  ...
  model = Model(inputs, x, name='nin_cnn')
  return model
...
if __name__ == "__main__":
  ...
  _ = compile_and_train(nin_cnn_model, 200, train_dataset, train_labels, valid_dataset, valid_labels)

  test_loss, test_acc = nin_cnn_model.evaluate(test_dataset, test_labels)
