...
from keras.layers import Conv2D, MaxPooling2D, GlobalAveragePooling2D, Activation, BatchNormalization, Dropout
...
def vgglike_cnn(inputs):
  ...
  x = BatchNormalization(axis=-1)(x)
  x = Conv2D(1024, kernel_size=(1,1), activation='relu')(x)
  x = BatchNormalization(axis=-1)(x)
  ...
  model = Model(inputs, x, name='vgglike_cnn')
  return model
...
if __name__ == "__main__":
  ...
  _ = compile_and_train(vgglike_cnn_model, 200, train_dataset, train_labels, valid_dataset, valid_labels)

  test_loss, test_acc = vgglike_cnn_model.evaluate(test_dataset, test_labels)
