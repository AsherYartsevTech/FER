...
def augment_by_mirror(dataset):
  augmented = np.ndarray(shape=(2 * dataset.shape[0], image_size, image_size), dtype=np.float32)
...
def augment_by_rotation(dataset):
  augmented = np.ndarray(shape=(3 * dataset.shape[0], image_size, image_size), dtype=np.float32)
  for i, image in enumerate(dataset):
    augmented[3*i, :, :] = image
...
def expand_dims(dataset):
  expanded = np.ndarray(shape=(dataset.shape[0], image_size, image_size, num_channels), dtype=np.float32)

  for i, image in enumerate(dataset):
    expanded[i,:,:,:] = np.expand_dims(image, axis=-1)
...
pickle_file = os.path.join(data_root, 'FER4dims.joblib')
